...
RESTAURANT_LIST = ["McDonalds", "Pastamania", "Dominos", "Al-Amans", "Koi"]

LOCATION_REGEX = "^(Yale-NUS|Tembusu College|RC 4|College of Alice and Peter Tan|Temasek Hall)$"
RESTAURANT_REGEX = "^(McDonalds|Pastamania|Dominos|Al-Amans|Koi)$"
    ...
    conv_handler_signup = ConversationHandler(
        ...
        states={
            LOCATION: [MessageHandler(
                Filters.regex(LOCATION_REGEX),
                location)],
            RESTAURANT: [MessageHandler(
                Filters.regex(RESTAURANT_REGEX),
                restaurant)],
        },
    )
